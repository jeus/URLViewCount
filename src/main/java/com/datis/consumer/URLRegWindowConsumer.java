 ...
 * @author jeus
 */
public class URLRegWindowConsumer extends Thread {

    LongDeserializer longDeserializer = new LongDeserializer();
    KryoDesrializer<WindowedUrlRegion> windowedDes;
    ...
    public URLRegWindowConsumer() {
    }

    public URLRegWindowConsumer(List<Properties> props, boolean logOn) {
        this.logOn = logOn;
        Map<String, Object> desPropsKey = new HashMap<>();
        desPropsKey.put("Kryo",WindowedUrlRegion.class);
...
//        countUser.poll(100);
        ...
        while (true) {
            ...
            ConsumerRecords<WindowedUrlRegion, RegionCount> ViewLogRecords = viewLog.poll(1000);
...
//            for (ConsumerRecord<String , String> rec : records) {
            if (ViewLogRecords.count() != 0) {
                if (logOn) {
                    for (ConsumerRecord<WindowedUrlRegion, RegionCount> rec : ViewLogRecords) {
                        strBuilder.append("ofs:").append(rec.offset());
                        strBuilder.append(" url:").append(rec.key().url);
                        strBuilder.append(" rgn:").append(rec.key().region);
    ...
    public static void main(String[] arg) {
        ...
        viewLog.put(ConsumerConfig.GROUP_ID_CONFIG, "urlregwindow");
...
//        props.add(propsByUser);
        URLRegWindowConsumer uRLRegWindowConsumer = new URLRegWindowConsumer(props, true);
        uRLRegWindowConsumer.start();
    }
